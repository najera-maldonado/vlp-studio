# ---------- foto_poro.pml ----------
# Carga automática según archivos en la carpeta

# Buscar archivo .pqr y cargar como modelo
python
import glob
pqr = glob.glob("*.pqr")[0]
...
cmd.load(pqr, "modelo")
cmd.load(dx, "pot")
# ❌ Borra cualquier barra CGO que venga de APBS Tools u otros
for obj in cmd.get_names("all"):
    if "color" in obj.lower() or "bar" in obj.lower():
        cmd.delete(obj)
python end
