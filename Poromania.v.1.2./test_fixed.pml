...
def calculate_pore_center_for_structure(structure_name):
    ...
    stored.coords = []
    # Obtener coordenadas de CA de los residuos seleccionados
    ...
    if stored.coords:
        ...
        center_5fold = calculate_pentamer_center(coords)
        return center_avg, center_5fold
...
def calculate_pore_vector(center_avg, center_5fold):
    # Calcula el vector de direccion del poro
    import numpy as np

    # Vector del centro pentamerico al centro promedio
    # (esto replica el calculo manual exitoso)
    vector = center_avg - center_5fold
...
def calculate_pore_vector(center_avg, center_5fold):
    ...
    # Vector del centro pentamérico al centro promedio
    # (esto replica el cálculo manual exitoso)
    vector = center_avg - center_5fold
...
for idx, mutations in enumerate(mutants_data, 1):
    ...
    center_avg, center_5fold = calculate_pore_center_for_structure(tag)

    # Calcular vector de dirección del poro
    pore_vector = calculate_pore_vector(center_avg, center_5fold)
